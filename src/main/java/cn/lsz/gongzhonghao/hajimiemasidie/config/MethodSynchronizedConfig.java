 ...
 */
@Component
public class MethodSynchronizedConfig implements ApplicationListener<ContextRefreshedEvent> {
        ...
        if(event.getApplicationContext().getParent()==null){
            ...
            for(Object bean : beans.values()){
                ...
                for (Method method : methods) {
                    //获取使用了自定义注解的方法
                    MethodSynchronizedAnnotation annotation = AnnotationUtils.findAnnotation(method, MethodSynchronizedAnnotation.class);
                    if(annotation != null){
                        //保存key中解析的结果
                        ...
                        try {
                            ...
                            //解析map key
                            paramList = keyHandler(annotation.keys(), sourceMethod);
